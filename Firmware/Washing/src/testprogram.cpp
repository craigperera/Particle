...
const int MACHINE_ON_NOT_LOCKED = TURN_ON + (SECONDS * 1);
const int START_WASHING = MACHINE_ON_NOT_LOCKED + SECONDS;
const int START_RINSING = START_WASHING + (SECONDS * 55);
const int START_SPINNING = START_RINSING + (SECONDS * 55);
const int START_PUMPING = START_SPINNING + (SECONDS * 55);
const int PUMPOUT_ENDED = START_PUMPING + (SECONDS * 55);
...
Readings loadTestPinState() {
    ...
    if (current >= WASHING_ENDED && current < TURN_OFF) {
        ...
        Serial.printlnf("Test Program Ended");
...
int setTestProgram(String command) {
    ...
    if (command == "start") {
        ...
        Serial.println("Test Program Aborted");
        lastEvent = 0;
        runTestProgram = false;
    }
